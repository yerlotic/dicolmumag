    ...
    char const * aSourcePathsAndNames)
{
    ...
    while ((p2 = strchr(p, MULTI_SEPARATOR)) != NULL)
    {
        lLen = p2 - p;
    ...
    int aAllowMultipleSelects ) /* 0 or 1 */
{
    ...
    if ( aAllowMultipleSelects && strchr(p, MULTI_SEPARATOR) )
    {
...
\n\tprint lFilesString[:-1]\n\"" ) ;
    ...
    if ( aAllowMultipleSelects && strchr(lBuff, MULTI_SEPARATOR) )
    {
    ...
    else if ( !fileExists(lBuff) )
    {
        ...
        p = strrchr(lBuff, MULTI_SEPARATOR);
        if ( !p ) p = lBuff ;
        else p ++ ;
        getPathWithoutFinalSlash( lString , p ) ;
